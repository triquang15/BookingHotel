  ...
  <li className='relative bg-white flex flex-col justify-between items-center shadow-md hover:shadow-xl rounded-md
    ...
    <Link className='contents' to={`/category/${listing.type}/${id}`}>
        ...
        <div className='w-full p-[10px]'>
          ...
          <p className='font-semibold mt-0 text-xl truncate text-[#c73939]'>{listing.name}</p>
          <p className='text-[#9d7b6b] mt-2 font-semibold'>${listing.offer ? listing.discountedPrice
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, ",")
            : listing.price
            .toString()
